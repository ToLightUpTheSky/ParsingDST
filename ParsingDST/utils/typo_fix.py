...
def typo_fix(slot_values, ontology, version="2.1"):
    ...
    open_slots = ['hotel-name', 
                     'train-destination', 'train-departure', "train-departure", "train-arriveby", 
                     'attraction-type', 'attraction-name',
                     'taxi-departure', 'taxi-destination',"taxi-leaveat", "taxi-arriveby",
                     'restaurant-name', 'restaurant-food', "restaurant-book time"]
    fixed = {}
    for slot, value in slot_values.items():
        ...
        if ':' in value: 
            ...
            try:
                if xx-2<0:
                    value = ("0"+value[xx-1]+':'+value[xx+1]+value[xx+2]).strip()
                else:
                    value = (value[xx-2]+value[xx-1]+':'+value[xx+1]+value[xx+2]).strip()
            except:
                pass
            if len(value) < 5:
                value = '0' + value
        ...
        if slot not in open_slots and value not in ontology[slot]:
            value = ''
